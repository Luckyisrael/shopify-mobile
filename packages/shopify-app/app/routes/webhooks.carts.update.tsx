import type { ActionFunctionArgs } from "react-router";
import { authenticate } from "../shopify.server";
import db from "../db.server";
import { logEventV2, EVENT_TYPES } from "../services/automation-v2.server";

export const action = async ({ request }: ActionFunctionArgs) => {
    const { topic, shop, payload } = await authenticate.webhook(request);

    console.log(`[Webhook] ${topic} for ${shop}`);

    if (!payload) return new Response();

    try {
        // Find merchant
        const merchant = await db.merchant.findUnique({ where: { shop } });
        if (!merchant) {
            console.error(`[Webhook] Merchant not found for shop ${shop}`);
            return new Response();
        }

        // Extract customer ID if available
        const customerId = payload.buyer_identity?.customer?.id;
        
        // Determine if cart is abandoned (no recent updates)
        const updatedAt = new Date(payload.updated_at);
        const now = new Date();
        const timeSinceUpdate = now.getTime() - updatedAt.getTime();
        const abandonedThreshold = 30 * 60 * 1000; // 30 minutes

        if (timeSinceUpdate > abandonedThreshold && payload.line_items?.length > 0) {
            // Cart is abandoned
            await logEventV2(
                merchant.id,
                EVENT_TYPES.CART_ABANDONED,
                {
                    cartId: payload.id,
                    cartToken: payload.token,
                    lineItemsCount: payload.line_items.length,
                    totalPrice: payload.total_price,
                    currency: payload.currency,
                },
                customerId
            );
        } else {
            // Cart is updated
            await logEventV2(
                merchant.id,
                EVENT_TYPES.CART_UPDATED,
                {
                    cartId: payload.id,
                    cartToken: payload.token,
                    lineItemsCount: payload.line_items.length,
                    totalPrice: payload.total_price,
                    currency: payload.currency,
                },
                customerId
            );
        }

    } catch (error) {
        console.error(`[Webhook] Error processing ${topic}:`, error);
    }

    return new Response();
};